...
pub fn validate_superadmin_token(token: &str) -> bool {
    let (valid_token, _) = get_superadmin_token();

    // Check if it matches the superadmin token directly
    ...
    if let Ok(claims) = jwt_auth.validate_token(token) {
        return JwtAuth::is_superadmin(&claims);
    }

    false
}

pub async fn dashboard_auth_middleware(request: Request<Body>, next: Next) -> Response {
    ...
    let token = cookie_header.and_then(|cookies| {
        cookies
            .split(';')
            .find(|c| c.trim().starts_with("wa_rs_token="))
            .map(|c| c.trim().trim_start_matches("wa_rs_token=").to_string())
    });

    match token {
        Some(t) if validate_superadmin_token(&t) => next.run(request).await,
        _ => {
            // Redirect to login
            Response::builder()
